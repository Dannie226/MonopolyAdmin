...
function threeToAmmo(three: THREE.Vector3 | THREE.Quaternion, ammo: Ammo.btVector3 | Ammo.btQuaternion | number) {
    if(typeof ammo == "object") ammo = Ammo.getPointer(ammo);
    three.toArray(Ammo.HEAPF32, ammo / 4);
}

function ammoToThree(three: THREE.Vector3 | THREE.Quaternion, ammo: Ammo.btVector3 | Ammo.btQuaternion | number) {
    if(typeof ammo == "object") ammo = Ammo.getPointer(ammo);
    three.fromArray(Ammo.HEAPF32, ammo / 4);
}

function destroyAll(...objs: Ammo.AmmoObj[]) {
...
class ConvexHull {
    ...
    public assigned: VertexList;
    public unassigned: VertexList;
    ...
    setFromPoints(points: THREE.Vector3[]) {
    ...
    intersectRay(ray: THREE.Ray, target: THREE.Vector3) {
    ...
    computeHorizon(eyePoint: THREE.Vector3, crossEdge: HalfEdge, face: Face, horizon: HalfEdge[]) {
...
class VertexNode {
    ...
    public prev: VertexNode;
    public next: VertexNode;
    public face: Face;
...
class VertexList {
    public head: VertexNode;
    public tail: VertexNode;
    constructor() {
        this.head = null;
        this.tail = null;
    }
...
function euclideanModulo(m: number, n: number) {
    return ((m % n) + n) % n;
}

function inputModulo(num: number, min: number, max: number) {
...
class Shape {
    ...
    constructor(geometry: THREE.BufferGeometry, shape: Ammo.btCollisionShape) {
...
class RigidBody extends THREE.EventDispatcher<any> {
    public mesh: THREE.Mesh;
    public body: Ammo.btRigidBody & {bodyWrapper: RigidBody};
    public motionState: Ammo.btMotionState;
    public isSleeping: boolean;
    public pointer: number;
    constructor(mesh: THREE.Mesh, body: Ammo.btRigidBody, motionState: Ammo.btMotionState) {
        super();
        this.mesh = mesh;
    ...
    applyImpulse(impulse: THREE.Vector3, relativeLocation: THREE.Vector3) {
    ...
    applyCentralImpulse(impulse: THREE.Vector3) {
        this.applyImpulse(impulse, new THREE.Vector3());
    }

    applyForce(force: THREE.Vector3, relativeLocation: THREE.Vector3) {
    ...
    setLinearFactor(factor: THREE.Vector3) {
        factor.toArray(Ammo.HEAPF32, this.pointer / 4 + 87);
    }

    setAngularFactor(factor: THREE.Vector3) {
        factor.toArray(Ammo.HEAPF32, this.pointer / 4 + 136)
    }

    setDamping(linearDamping: number, angularDamping: number) {
    ...
    setTransform(position: THREE.Vector3, rotation: THREE.Quaternion) {
        const v = new Ammo.btVector3(), q = new Ammo.btQuaternion(1, 0, 0, 0);
    ...
    onBeforeUpdate(world: World) {}

    onAfterUpdate(world: World) {}
}

class World<T extends Ammo.btCollisionWorld = Ammo.btCollisionWorld> {
    public bodies: RigidBody[];
    public constraints: Constraint[];
    public scene: THREE.Scene;
    public world: T;
    public tmpTrans: Ammo.btTransform;
    public contactResult: Ammo.ConcreteContactResultCallback & {checkObj: Ammo.btRigidBody};
    constructor(scene: THREE.Scene, world: T) {
        this.bodies = [];
        this.constraints = [];
        this.scene = scene;
        this.world = world;
        ...
        this.contactResult = new Ammo.ConcreteContactResultCallback() as any;
        this.contactResult.checkObj = null;
        (this.contactResult as any).addSingleResult = function(cp, colObj0Wrap, partId0, index0, colObj1Wrap, partId1, index1) {
    ...
    enableGImpact() {
        if((Ammo as any).btGImpactCollisionAlgorithm) {
            (Ammo as any).btGImpactCollisionAlgorithm.prototype.registerAlgorithm(this.world.getDispatcher());
        } else {
...
class Constraint<T extends Ammo.btTypedConstraint = Ammo.btTypedConstraint> {
    public bodyA: RigidBody;
    public bodyB: RigidBody;
    public constraint: T;
    constructor(bodyA: RigidBody, bodyB: RigidBody, constraint: T) {
        this.bodyA = bodyA;
        this.bodyB = bodyB;
...
class TrimeshShape extends Shape {
    public trimesh: Ammo.btTriangleMesh;
    constructor(geometry: THREE.BufferGeometry, removeDuplicateVertices = false) {
...
class Material {
    ...
    constructor(material: THREE.Material, friction = 0.25, restitution = 0.0) {
...
class Body extends RigidBody {
    public shape: Shape;
    constructor(shape: Shape, mass: number, material: Material, position = new THREE.Vector3, rotation = new THREE.Quaternion) {
        const mesh = new THREE.Mesh(shape.geometry, material.material);

        const transform = new Ammo.btTransform();
        transform.setIdentity();

        const p = new Ammo.btVector3(), q = new Ammo.btQuaternion(1, 0, 0, 0);
...
interface InfinitePlaneOptions {
    meshWidth?: number;
    meshHeight?: number;
    meshWidthSegments?: number;
    meshHeightSegments?: number;
}

class InfinitePlane extends RigidBody {
    constructor(material: Material, normal = new THREE.Vector3(0, 1, 0), distance = 0, options: InfinitePlaneOptions = {}) {
...
class DynamicsWorld extends World<Ammo.btDiscreteDynamicsWorld> {
    constructor() {
    ...
    add(body: RigidBody, collisionGroup = 1, collisionMask = 1) {
        this.bodies.push(body);
        this.world.addRigidBody(body.body, collisionGroup, collisionMask);
        this.scene.add(body.mesh);
    }

    remove(body: RigidBody) {
...
class SoftBodyWorld extends World<Ammo.btSoftRigidDynamicsWorld> {
    public softbodies: SoftBody[];
    constructor() {
    ...
    addSoftBody(body: SoftBody, collisionGroup: number, collisionMask: number) {
    ...
    update(dt: number) {
        ...
        for (const body of this.bodies) {
            ...
            (this.contactResult as any).checkObj = a;
            this.world.contactTest(a, this.contactResult);
...
class SoftBody extends THREE.EventDispatcher {
    public body: Ammo.btSoftBody;
    public mesh: THREE.Mesh;
    public config: Ammo.Config;
    constructor(softbody: Ammo.btSoftBody, mesh: THREE.Mesh) {
        super();
        this.body = softbody;
        this.mesh = mesh;
...
class SoftVolume extends SoftBody {
    ...
    constructor(geometry: THREE.BufferGeometry, mass: number, material: Material, world: SoftBodyWorld, position = new THREE.Vector3, rotation = new THREE.Quaternion) {
        ...
        (pos as any).usage = THREE.StreamDrawUsage;
        (nor as any).usage = THREE.StreamDrawUsage;
    ...
    setDrag(drag: number) {
        this.config.set_kDG(drag);
    }

    setFriction(friction: number) {
        this.config.set_kDF(friction)
    }
...
class Cloth extends SoftBody {
    constructor(planeGeometry: THREE.PlaneGeometry, mass: number, material: Material, world: SoftBodyWorld, position = new THREE.Vector3, rotation = new THREE.Quaternion) {
...
class Rope extends SoftBody {
    constructor(startPoint: THREE.Vector3, endPoint: THREE.Vector3, segments: number, mass: number, material: Material, world: SoftBodyWorld) {
        const geo = new THREE.BufferGeometry();
        ...
        const helpers = new Ammo.btSoftBodyHelpers();
        const start = new Ammo.btVector3(startPoint.x, startPoint.y, startPoint.z);
        const end = new Ammo.btVector3(endPoint.x, endPoint.y, endPoint.z);
...
class Point2PointConstraint extends Constraint<Ammo.btPoint2PointConstraint> {
    constructor(bodyA: RigidBody, bodyB: RigidBody, localA: THREE.Vector3, localB: THREE.Vector3) {
...
class FixedConstraint extends Constraint<Ammo.btFixedConstraint> {
    constructor(bodyA: RigidBody, bodyB: RigidBody, posLocA: THREE.Vector3, rotLocA: THREE.Quaternion, posLocB: THREE.Vector3, rotLocB: THREE.Quaternion) {
        ...
        const pLocA = new Ammo.btVector3(),
            qLocA = new Ammo.btQuaternion(1, 0, 0, 0);
        const pLocB = new Ammo.btVector3(),
            qLocB = new Ammo.btQuaternion(1, 0, 0, 0);
...
class HingeConstraint extends Constraint<Ammo.btHingeConstraint> {
    constructor(bodyA: RigidBody, bodyB: RigidBody, localA: THREE.Vector3, localB: THREE.Vector3, hingeAxisA: THREE.Vector3, hingeAxisB: THREE.Vector3) {
        const locA = new Ammo.btVector3(),
            locB = new Ammo.btVector3();
...
class SpringConstraint extends Constraint<Ammo.btGeneric6DofSpringConstraint> {
    public ptr: number;
    constructor(bodyA: RigidBody, bodyB: RigidBody, posLocA: THREE.Vector3, posLocB: THREE.Vector3, rotLocA: THREE.Quaternion, rotLocB: THREE.Quaternion) {
        const locA = new Ammo.btTransform(),
            locB = new Ammo.btTransform();
        locA.setIdentity();
        locB.setIdentity();
    ...
    setSpring(index: number, equilibrium: number, stiffness: number, damping: number) {
    ...
    setAngularYLimit(upper: number, lower: number) {
    ...
    setAngularZLimit(upper: number, lower: number) {
    ...
    setLinearXSpring(equilibrium: number, stiffness: number, damping = 0.1) {
        this.setSpring(0, equilibrium, stiffness, damping);
    }

    setLinearYSpring(equilibrium: number, stiffness: number, damping = 0.1) {
        this.setSpring(1, equilibrium, stiffness, damping);
    }

    setLinearZSpring(equilibrium: number, stiffness: number, damping = 0.1) {
        this.setSpring(2, equilibrium, stiffness, damping);
    }

    setAngularXSpring(equilibrium: number, stiffness: number, damping = 0.1) {
        this.setSpring(3, equilibrium, stiffness, damping);
    }

    setAngularYSpring(equilibrium: number, stiffness: number, damping = 0.1) {
        this.setSpring(4, equilibrium, stiffness, damping);
    }

    setAngularZSpring(equilibrium: number, stiffness: number, damping = 0.1) {
        this.setSpring(5, equilibrium, stiffness, damping);
    }
}

class ConeTwistConstraint extends Constraint<Ammo.btConeTwistConstraint> {
    constructor(bodyA: RigidBody, bodyB: RigidBody, posLocA: THREE.Vector3, posLocB: THREE.Vector3, rotLocA: THREE.Quaternion, rotLocB: THREE.Quaternion) {
        const locA = new Ammo.btTransform(),
            locB = new Ammo.btTransform();
        locA.setIdentity();
    ...
    setTwistLimit(limit: number) {
        this.constraint.setLimit(3, limit);
    }

    setSpan1Limit(limit: number) {
        this.constraint.setLimit(4, limit);
    }

    setSpan2Limit(limit: number) {
...
function DebugDrawer(world: World, bufferSize = 100000) {        
    this.world = world;
    ...
    (drawer as any).enable = this.enable.bind(this);
    (drawer as any).disable = this.disable.bind(this);
    (drawer as any).update = this.update.bind(this);
